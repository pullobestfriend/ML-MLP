...
class Layer(object):
    ...
    def up_bias(self, learn_rate):
        bias_error_derivative = 0
        for neuron in self.neurons:
...
class MLP(object):
    ...
    def backpropagate_all(self, target):
        ...
        for neuron in self.hidden_layers.neurons:
            for idx, _ in enumerate(neuron.weights):
                ratio = neuron.delta * neuron.get_de_clean_out_per_dweight(idx)
                neuron.weights[idx] -= self.learn_rate * ratio
        self.out_layer.up_bias(self.learn_rate)
        self.hidden_layers.up_bias(self.learn_rate)
    ...
    def test(self,data):
        ...
        for feature in data:
            ...
            if int(self.activation_function(join[0][1])) == int(join[1][1]) and int(self.activation_function(join[0][0])) == int(join[1][0]):
    ...
    def plot(self):
        ...
        plot.plot(plot_index,[val[0] for val in self.graphof_sum_square_error])
...
mlp = MLP(data=data, input_size=4, hidden_layers_size=5,out_layer_size=2, epoch=2000, learn_rate=0.1).run()